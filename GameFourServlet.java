    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = JDBCUtil.getConnection();
            ...
            if (lunches.isEmpty()) {
                ...
                out.println("<style>");
                out.println("body { font-family: 'Comic Sans MS', cursive, sans-serif; text-align: center; margin-top: 50px; }");
                out.println("</style>");
                out.println("</head>");
                out.println("<body>");
        ...
        out.println("body { background: #f7f8fc; }");
        out.println("canvas { transition: 2s; pointer-events: none; }");
        out.println("button { background: #febf00; margin-top: 1rem; padding: .8rem 1.8rem; border: none; font-size: 1.5rem; font-weight: bold; border-radius: 5px; transition: .2s; cursor: pointer; }");
        out.println("button:active { background: #444; color: #f9f9f9; }");
        out.println("div { width: 380px; display: flex; align-items: center; flex-direction: column; position: relative; }");
        out.println("div::before { content: ''; position: absolute; width: 10px; height: 50px; border-radius: 5px; background: #000; top: -20px; left: 50%; transform: translateX(-50%); z-index: 22; }");
        out.println("</style>");
        out.println("</head>");
        out.println("<body>");
        out.println("<div>");
        out.println("<canvas width='380' height='380'></canvas>");
        out.println("<button onclick='rotate()'>룰렛 돌리기</button>");
        out.println("<button onclick=\"window.location.href='getLunches'\" style=\"margin-top: 20px;\">Main</button>");
        out.println("</div>");
        out.println("<script>");
        ...
        out.println("const colors = ['#dc0936', '#e6471d', '#f7a416', '#efe61f', '#60b236', '#209b6c', '#169ed8', '#3f297e', '#87207b', '#be107f', '#e7167b'];");
